...
function getStatusDotClass(status: ProviderConnectionStatus): string {
  switch (status) {
    ...
    case 'stopped':
      return 'bg-[var(--pd-status-stopped)] shadow-[0_0_8px_var(--pd-status-stopped)]';
    case 'unknown':
      return 'bg-[var(--pd-status-terminated)] shadow-[0_0_8px_var(--pd-status-terminated)]';
    case 'starting':
    case 'stopping':
      return 'bg-[var(--pd-status-starting)] shadow-[0_0_8px_var(--pd-status-starting)]';
    default:
      return 'bg-[var(--pd-status-stopped)] shadow-[0_0_8px_var(--pd-status-stopped)]';
  }
}
...
<div class="inline-flex items-center gap-1.5">
    <div 
        class="rounded-full {getSizeClass(size)} {getStatusDotClass(status)}"
        aria-label="Status indicator"
        role="status"
    ></div>
    {#if showText}
        <span class="text-xs {getStatusTextClass(status)}">{getStatusLabel(status)}</span>
    {/if}
</div>
