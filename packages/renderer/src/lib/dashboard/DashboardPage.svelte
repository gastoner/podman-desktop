...
function getDefaultDashboardItems(): ListOrganizerItem[] {
  return $dashboardPageRegistry.map(entry => ({
    id: entry.id,
    label: entry.id,
    enabled: true,
...
async function loadDashboardConfiguration(): Promise<ListOrganizerItem[]> {
  if (tablePersistence.storage) {
    ...
    const loadedItems = await tablePersistence.storage.load('dashboard', sectionNames);
...
$effect(() => {
  if (isInitialized && dashboardSections.length > 0) {
    ...
    const updatedEntries = convertFromListOrganizerItems(orderedSections, $dashboardPageRegistry);
...
async function resetDashboardLayout(): Promise<void> {
  if (tablePersistence.storage) {
    try {
      ...
      dashboardSections = await tablePersistence.storage.reset('dashboard', defaultSections);
      dashboardOrdering.clear();
      ...
      if ($dashboardPageRegistry.length > 0) {
        const updatedEntries = convertFromListOrganizerItems(dashboardSections, $dashboardPageRegistry);
        dashboardPageRegistry.set(updatedEntries);
      }
    } catch (error: unknown) {
    ...
    <div class="min-w-full flex-1">
      ...
      <div class="px-5 space-y-5 h-full">
        ...
        {#each sortedDashboardRegistry as dashboardRegistryItem (dashboardRegistryItem.id)}
          {@const Section = dashboardRegistryItem.component as Component}
          <Section />
        {/each}
      </div>
    </div>
